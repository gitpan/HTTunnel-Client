...
=item remote_proto = <proto>

Protocol for the tunnelled connection. Default is 'tcp', which
happens to be the only implemented value.

=item remote_port = <port nb.>

Remote port number. This directive is mandatory unless the server is
configured with HTTunnelForceHost.

=item remote_host = <hostname or ip address>

Remote host. This directive if mandatory unless the server is
configured with HTTunnelForcePort.
...
this value if your proxy interrupting the HTTP requests abruptly,
causing L<HTTunnel::Client> to not function corectly. The default
is 15 seconds.
...
L<HTTunnel::Client>, C<LWP::UserAgent>
